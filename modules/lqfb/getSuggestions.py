...
#############################################################################
# Purpose:
#   List the suggestions of a LQFB initiative by given id.
#
# Usage:
#   getSuggestions.py INSTANCE INITIATIVEID
#
...
if INSTANCE == 'bund':
    url = "https://lqfb.piratenpartei.de/pp/api/suggestion.html&key=" + lqfb_keys['keys']['bund'] + "&initiative_id=" + INITIATIVEID + "&api_engine=json"
else:
    url = "https://lqpp.de/" + INSTANCE + "/webmcp-wrapper.lua?_webmcp_urldepth=1&_webmcp_module=api&_webmcp_view=suggestion&key=" + lqfb_keys['keys'][INSTANCE] + "&initiative_id=" + INITIATIVEID + "&api_engine=json"
...
for entry in obj:
    if INSTANCE == 'bund':
        entry.update({'url' : 'http://pplf.de/a' + str(entry['id'])})
    else:
        entry.update({'url' : 'http://' + INSTANCE + '.pplf.de/a' + str(entry['id'])})
