...
#############################################################################
# Purpose:
#   Returns the pad details of a pad identified by a pad group and an
#   identifier.
#
# Usage:
#   getPadById.php PADGROUP PADID
#
#   PADGROUP: A pad group at the http://www.piratenpad.de server.
#   PADID: An identifier of a pad.
#
# Note:
#   For PADGROUP=meck-pom and PADID=366, the URL of the pad would be
#   https://meck-pom.piratenpad.de/366.
#
# Files:
...
if ($argc != 3) die("Exactly two parameters must be given!\n");
$PADGROUP = $argv[1];
$PADID = $argv[2];
...
function getpads($pad_group, $pad_id) {
    global $postfields;
    ...
    for ($i = 0; $i < count($table_titles); $i++) {
        ...
        if (strcmp(str_replace("/", "", $table_urls[$i]->href), $pad_id) == 0) {
            ...
            if ($public) {
                ...
                // make JSON look nicer by looking into the array
                $json = getJSON($pad);
